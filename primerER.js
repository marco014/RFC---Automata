...
export function validarYMostrar() {
    var cadena = document.getElementById('cadenaInput').value.toUpperCase();

    if (cadena.length === 4) {
        var resultado = validarPrimeraExpresion(cadena);
        if (resultado) {
            document.getElementById('resultado').innerText = cadena + ' - Cadena Valida.';
        } else {
            document.getElementById('resultado').innerText = cadena + ' - Cadena no valida. No se hace.';
        }
    } else {
        document.getElementById('resultado').innerText = cadena + ' - La cadena debe tener 4 letras. Cadena no valida.';
    }
}

function validarPrimeraExpresion(cadena) {
    var expresionRegular = /^[A-Za-z]{4}$/;
    var resultado = expresionRegular.test(cadena);

    if (resultado) {
        console.log(cadena + ' - Cadena Valida.');
        generarAutomata(cadena);
        return true;
    } else {
        console.log(cadena + ' - Cadena no valida. No se hace.');
        return false;
    }
}


function generarAutomata(cadena) {
    d3.select("svg").remove(); // Elimina el SVG existente

    // Datos del automata
    var estados = ["q0", "q1", "q2", "q3", "q4"]; // estados
    var transiciones = []; // lineas
    ...
    var svg = d3.select(".er1").append("svg")
        .attr("width", 500) // Aumentar el ancho para mejorar la visualización
    ...
    console.log('Visualización del automata.');
}
